...
{
	return discriminate<FORMATS...>(format);
}
...
{
	return printFormatSpecific(format, graph, reducedGraph, mathematical, gateCost, humanLong, human, humanShort, cpp, verilog, vhdl);
}
...
{
	return printFormatSpecific(format, graph, mathematical, gateCost, human, cpp, verilog, vhdl);
}
...
{
	return printFormatSpecific(format, graph, mathematical, human, cpp, verilog, vhdl);
}
...
{
	return printFormatSpecific(format, graph, mathematical, human, program);
}
...
{
	return printFormatSpecific(format, cpp, verilog, vhdl);
}
...
{
	for (const Solution &solution : solutions)
		if (std::ranges::any_of(solution, [](const Implicant &implicant){ return implicant.getBitCount() != 0; }))
			return true;
	return false;
}
...
bool OutputComposer::isOptimizedSumWorthPrintingInGeneral(const OptimizedSolutions::id_t sumId) const
{
	...
	if (std::ranges::count(optimizedSolutions->getFinalSums(), sumId) >= 2 && optimizedSolutions->getSum(sumId).size() >= 2)
		return true;
...
bool OutputComposer::isOptimizedSumWorthPrinting(const OptimizedSolutions::id_t sumId) const
{
	...
	case OF::MATHEMATICAL:
		return isOptimizedSumWorthPrintingInGeneral(sumId) && (optimizedSolutions->getIdUseCount(sumId) + std::ranges::count(optimizedSolutions->getFinalSums(), sumId) >= 2);
	default:
...
{
	const OF outputFormat = useHumanAnyway ? OF::HUMAN : format;
	if (optimizedSolutions->isProduct(id))
		printFormatSpecific(outputFormat, 's', 'p', BLANK, "prods");
	else
		printFormatSpecific(outputFormat, NEXT, 's', BLANK, "sums");
	printFormatSpecific(outputFormat, NEXT, BLANK, NEXT, NEXT, '[', '(');
...
{
	const auto print = [this](const char formal, const char ascii, const char programming, const char names){ return printChoiceSpecific<OO, OO::FORMAL, OO::ASCII, OO::PROGRAMMING, OO::NAMES>(operators, formal, ascii, programming, names); };
	switch (value.get())
	{
...
constexpr OF OutputComposer::mapOutputOperators(const OO operators)
{
	switch (operators)
	{
	case OO::FORMAL:
...
OF OutputComposer::getOperatorsStyle() const
{
	return isProgramming()
			? format
			: mapOutputOperators(operators);
}

void OutputComposer::printBool(const bool value, const bool strictlyForCode) const
{
	const OF outputFormat = strictlyForCode ? OF::CPP : getOperatorsStyle();
	if (value)
		printFormatSpecific(outputFormat, 'T', "\u22A4", "TRUE", "true", '1', "'1'");
	else
		printFormatSpecific(outputFormat, 'F', "\u22A5", "FALSE", "false", '0', "'0'");
}

void OutputComposer::printNot() const
{
	printFormatSpecific(getOperatorsStyle(), '~', "\u00AC", "NOT ", NEXT, '!', "not ");
}
...
void OutputComposer::printAnd(const bool spaces) const
{
	if (spaces)
		o << ' ';
	printFormatSpecific(getOperatorsStyle(), "/\\", "\u2227", "AND", "&&", '&', "and");
	if (spaces)
		o << ' ';
}
...
void OutputComposer::printOr(const bool spaces) const
{
	if (spaces)
		o << ' ';
	printFormatSpecific(getOperatorsStyle(), "\\/", "\u2228", "OR", "||", '|', "or");
	if (spaces)
		o << ' ';
}
...
void OutputComposer::printBanner() const
{
	...
	if (std::ranges::any_of(options::allOptions, [](const options::Option *const option){ return option->isSet(); }))
	{
		o << " (with options:";
...
void OutputComposer::printGraphNegatedInputs(const Solution &solution, const std::size_t functionNum) const
{
	...
	if (std::ranges::any_of(negatedInputs, [](const std::vector<std::size_t> &x){ return !x.empty(); }))
	{
...
void OutputComposer::printGraphSum(const Solution &solution, const std::size_t functionNum) const
{
	...
	o << "{\n";
	{
		...
		const bool hasParents = isFullGraph || (solution.size() >= 2 && std::ranges::any_of(solution, [](const Implicant &x){ return x.getBitCount() >= 2; }));
		{
			const auto sanitizeGuard = o.startSanitize();
...
void OutputComposer::printOptimizedGraphSumProducts(const OptimizedSolutions::id_t sumId) const
{
	...
	else
	{
		...
		const auto eraseBegin = std::remove_if(sum.begin(), sum.end(), [this](const OptimizedSolutions::id_t id){ return !optimizedSolutions->isProduct(id) || isOptimizedProductWorthPrintingInGeneral(id); }), eraseEnd = sum.end();
		sum.erase(eraseBegin, eraseEnd);
	}
	First first;
