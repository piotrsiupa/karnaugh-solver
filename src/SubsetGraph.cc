	...
	for (const set_t &set : sets)
	{
		...
		for (std::size_t i = 0; i != setCopy.size();)
		{
			...
			else
			{
				...
				std::ranges::set_difference(groups[foundGroupIndex].cbegin(), groups[foundGroupIndex].cend(), setCopy.cbegin(), setCopy.cend(), std::back_inserter(groupRejects));
				if (!groupRejects.empty())
				{
					typename group_t::const_iterator currentRejectIt = groupRejects.cbegin(), endRejectIt = groupRejects.cend();
					const auto [eraseBegin, eraseEnd] = std::ranges::remove_if(groups[foundGroupIndex].begin(), groups[foundGroupIndex].end(), [&currentRejectIt, endRejectIt](const VALUE_T x){ if (currentRejectIt == endRejectIt || x != *currentRejectIt) return false; ++currentRejectIt; return true; });
					groups[foundGroupIndex].erase(eraseBegin, eraseEnd);
				...
				const auto [eraseBegin, eraseEnd] = std::ranges::remove_if(setCopy.begin(), setCopy.end(), [&currentValueIt, endValueIt](const VALUE_T x){ if (currentValueIt == endValueIt || x != *currentValueIt) return false; ++currentValueIt; return true; });
				setCopy.erase(eraseBegin, eraseEnd);
			}
		}
		if (!setCopy.empty())
		{
...
void SubsetGraph<VALUE_T, CONTAINER>::makeInitialSetHierarchy(const groupedSets_t &groupedSets)
{
	...
	for (std::size_t i = 0; i != groupedSets.size(); ++i)
	{
		...
		for (std::size_t j = i + 1; j != groupedSets.size(); ++j)
		{
			...
			std::ranges::set_intersection(groupedSets[i].cbegin(), groupedSets[i].cend(), groupedSets[j].cbegin(), groupedSets[j].cend(), std::back_inserter(partialSet));
...
void SubsetGraph<VALUE_T, CONTAINER>::trimSetHierarchy()
{
	...
	const auto [eraseBegin, eraseEnd] = std::ranges::remove_if(groupedSetHierarchy.begin(), groupedSetHierarchy.end(), [](const GroupedSetHierarchyEntry &entry){ return entry.setIds.empty(); });
	groupedSetHierarchy.erase(eraseBegin, eraseEnd);
...
void SubsetGraph<VALUE_T, CONTAINER>::addMoreEdgesToSetHierarchy()
{
	for (GroupedSetHierarchyEntry &entry0 : groupedSetHierarchy)
	{
		for (std::size_t j = 0; j != groupedSetHierarchy.size(); ++j)
		{
			...
			{
				if (std::ranges::find(entry0.subsets.cbegin(), entry0.subsets.cend(), j) == entry0.subsets.cend() && std::ranges::includes(entry0.groupIds.cbegin(), entry0.groupIds.cend(), groupedSetHierarchy[j].groupIds.cbegin(), groupedSetHierarchy[j].groupIds.cend()))
				{
...
void SubsetGraph<VALUE_T, CONTAINER>::removeRedundantEdgesFromSetHierarchy()
{
	for (auto iter = groupedSetHierarchy.rbegin(); iter != groupedSetHierarchy.rend(); ++iter)
	{
		if (!iter->setIds.empty())
		{
			...
			const auto [eraseBegin, eraseEnd] = std::ranges::remove_if(iter->subsets.begin(), iter->subsets.end(), [&subsetsOfSubsets = std::as_const(subsetsOfSubsets)](const std::size_t x) { return std::ranges::find(subsetsOfSubsets.cbegin(), subsetsOfSubsets.cend(), x) != subsetsOfSubsets.cend(); });
			iter->subsets.erase(eraseBegin, eraseEnd);
...
typename SubsetGraph<VALUE_T, CONTAINER>::setHierarchy_t SubsetGraph<VALUE_T, CONTAINER>::ungroupSetHierarchy() const
{
	...
	for (const GroupedSetHierarchyEntry &groupedSetHierarchyEntry : groupedSetHierarchy)
	{
		...
		std::ranges::sort(ungroupedValues.begin(), ungroupedValues.end());
