...
namespace options
{
	...
	Number<std::size_t> maxRoughWidth("rough-width", "(?:m(?:ax?)?[-_ ])?r(?:ou(?:gh)?)?(?:[-_ ]h(?:eur(?:is(?:t(?:ics?)?)?)?)?)?[-_ ](?:w(?:i(?:d(?:ths?)?)?)?|e(?:l(?:e(?:m(?:en(?:ts?)?)?)?)?)?|i(?:n(?:p(?:u(?:ts?)?)?)?)?|r(?:o(?:o(?:ts?)?)?)?)|m?rh?(?:d|(?:n|s[ngs]s)s)", 'w', 0, SIZE_MAX - 1, SIZE_MAX);
	...
	namespace
	{
		...
		const std::vector<Option*> Parser::allOptions = {
					...
					&skipOptimization, &optimizationHeuristics, &maxRoughDepth, &maxRoughWidth,
				};
