...
namespace options
{
	...
	Mapped<OptimizationHeuristic> optimizationHeuristics("cse-heuristic", "(?:(?:c(?:s|s?e)e|c(?:om(?:m(?:on)?)?)?[-_ ](?:s(?:ub(?:ex(?:pres(?:s(?:ions?)?)?)?)?)?|(?:s(?:ub)?[-_ ])?e(?:x(?:pres(?:s(?:ions?)?)?)?)?)[-_ ]e(?:lim(?:in(?:ations?)?)?|rad(?:ic(?:ations?)?)?)?)|o(?:pt(?:im(?:iz(?:ations?)?)?)?)?)[-_ ]h(?:eur(?:is(?:tics?)?)?)?|(?:c(?:(?:s|s?e)e)?|o)h", 'c', {
			{"brute-force", "(?:(?:f(?:ull)?[-_ ])?b(?:rute)?(?:[-_ ]f(?:or(?:ces?)?)?)?|f?bf)|s(?:low)?", OptimizationHeuristic::BRUTE_FORCE},
			{"exhaustive", "(?:e(?:x(?:hau(?:s(?:t(?:ive)?)?)?)?)?)|(?:(?:(?:a(?:l(?:m(?:ost)?)?)?|p(?:ar(?:t(?:ial)?)?)?|i(?:n(?:com(?:plete)?)?)?)[-_ ])b(?:rute)?(?:[-_ ]f(?:or(?:ces?)?)?)?|[api]bf?)", OptimizationHeuristic::EXHAUSTIVE},
